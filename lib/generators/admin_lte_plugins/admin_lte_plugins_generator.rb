class AdminLtePluginsGenerator < Rails::Generators::Base
  ...
  def install_ckeditor
    add_plugin('ckeditor', 'js', 'jquery.dataTables')
    add_plugin('ckeditor', 'js', 'dataTables.bootstrap')
    add_plugin('ckeditor', 'css', 'contents')
    
    plugin_directory = File.expand_path('../templates', __FILE__) + '/ckeditor'
    directory "#{plugin_directory}/adapters", 'vendor/assets/javascripts/adapters'
  end
